...
export default function Home() {
  ...
  const [windows, setWindows] = useState<WindowState[]>([
    { id: 'chat', title: '◈ Z Agent', x: 60, y: 50, w: 560, h: 520, z: 10, minimized: false },
    { id: 'terminal', title: 'Terminal — real bash', x: 650, y: 50, w: 600, h: 360, z: 5, minimized: false },
    { id: 'files', title: 'File Browser', x: 650, y: 430, w: 600, h: 280, z: 4, minimized: false },
    { id: 'monitor', title: 'System Monitor', x: 60, y: 590, w: 560, h: 180, z: 3, minimized: false },
    { id: 'settings', title: '⚙ Settings', x: 200, y: 100, w: 500, h: 480, z: 20, minimized: true },
  ])
  ...
  const renderWindow = (w: WindowState, content: React.ReactNode) => {
    ...
    return (
      <div key={w.id} style={{
        ...
        background: tc.surface, border: `1px solid ${activeWin === w.id ? accent : tc.surfaceLight}`,
        borderRadius: '6px', boxShadow: activeWin === w.id ? `0 8px 32px ${accent}30` : '0 4px 16px rgba(0,0,0,0.5)',
        zIndex: w.z, display: 'flex', flexDirection: 'column', overflow: 'hidden',
      }} onMouseDown={() => focusWindow(w.id)}>
        <div style={{
          background: activeWin === w.id ? `${accent}30` : tc.surfaceLight, padding: '6px 12px',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'move',
          borderBottom: `1px solid ${tc.surfaceLight}`, fontSize: '0.78rem', color: activeWin === w.id ? accent : '#888',
        ...
        <div onMouseDown={(e) => startResize(e, w.id)} style={{
          ...
          cursor: 'nwse-resize', background: `linear-gradient(135deg, transparent 50%, ${accent} 50%)`,
  ...
  const terminalContent = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div ref={outputRef} style={{ flex: 1, background: '#000', padding: '8px', overflowY: 'auto', whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontSize: '0.78rem', lineHeight: '1.4', color: accent, fontFamily: 'monospace' }}>
        {output || (connected ? '' : 'Connecting...')}
      </div>
      <div style={{ display: 'flex', gap: '0.4rem', padding: '6px', background: tc.bg, borderTop: `1px solid ${tc.surfaceLight}`, flexWrap: 'wrap' }}>
        {quickCmds.slice(0, 5).map(c => <button key={c} onClick={() => sendCmd(c)} style={{ background: tc.surfaceLight, color: accent, border: `1px solid ${tc.surfaceLight}`, padding: '2px 8px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.7rem', cursor: 'pointer' }}>{c.length > 16 ? c.slice(0, 14) + '..' : c}</button>)}
      </div>
      <form onSubmit={sendCommand} style={{ display: 'flex', gap: '0.4rem', padding: '6px', background: '#000', borderTop: `1px solid ${tc.surfaceLight}` }}>
        <span style={{ color: accent, alignSelf: 'center', fontFamily: 'monospace', fontSize: '0.8rem' }}>$</span>
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="bash command..." style={{ flex: 1, background: '#000', color: accent, border: `1px solid ${tc.surfaceLight}`, padding: '4px 8px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.8rem', outline: 'none' }} />
        <button type="submit" style={{ background: accent, color: '#000', border: 'none', padding: '4px 12px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.75rem', cursor: 'pointer', fontWeight: 'bold' }}>RUN</button>
      </form>
    </div>
  )

  const chatContent = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: tc.bg }}>
      {/* Minimal model indicator */}
      <div style={{ padding: '4px 10px', background: '#000', borderBottom: `1px solid ${tc.surfaceLight}`, display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', fontFamily: 'monospace' }}>
        <span style={{ color: '#666' }}>{currentModelName}{customPrompt && ' · custom prompt'}</span>
        <span style={{ color: '#666' }}>{chatMessages.length} msgs</span>
      </div>
      {/* Messages */}
      <div ref={chatRef} style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {chatMessages.length === 0 && (
          <div style={{ textAlign: 'center', color: '#666', fontSize: '0.8rem', marginTop: '40px', padding: '0 20px' }}>
            <div style={{ fontSize: '2rem', marginBottom: '12px', color: accent }}>◈</div>
            <div style={{ color: accent, fontWeight: 'bold', marginBottom: '8px' }}>Z is here.</div>
      ...
      <form onSubmit={sendChat} style={{ display: 'flex', gap: '6px', padding: '8px', background: '#000', borderTop: `1px solid ${tc.surfaceLight}` }}>
        <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="message Z..." disabled={chatLoading} style={{ flex: 1, background: tc.bg, color: '#ddd', border: `1px solid ${tc.surfaceLight}`, padding: '6px 10px', borderRadius: '4px', fontFamily: 'monospace', fontSize: '0.8rem', outline: 'none' }} />
        <button type="submit" disabled={chatLoading || !chatInput.trim()} style={{ background: chatLoading ? '#333' : accent, color: '#000', border: 'none', padding: '6px 14px', borderRadius: '4px', fontFamily: 'monospace', fontSize: '0.75rem', cursor: chatLoading ? 'not-allowed' : 'pointer', fontWeight: 'bold' }}>SEND</button>
  ...
  const settingsContent = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: tc.bg, overflowY: 'auto' }}>
      <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {/* BYOK Section */}
        <div>
          <div style={{ fontSize: '0.75rem', color: accent, fontWeight: 'bold', marginBottom: '8px', borderBottom: `1px solid ${tc.surfaceLight}`, paddingBottom: '4px' }}>API KEYS (BYOK)</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <input type="text" value={settings.baseUrl} onChange={(e) => setSettings({ ...settings, baseUrl: e.target.value })} placeholder="API Base URL (e.g. https://api.openai.com/v1)" style={{ background: '#000', color: '#ddd', border: `1px solid ${tc.surfaceLight}`, padding: '6px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.72rem', outline: 'none' }} />
            <div style={{ display: 'flex', gap: '4px' }}>
              <input type="password" value={settings.apiKey} onChange={(e) => setSettings({ ...settings, apiKey: e.target.value })} placeholder="API Key (sk-...)" style={{ flex: 1, background: '#000', color: '#ddd', border: `1px solid ${tc.surfaceLight}`, padding: '6px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.72rem', outline: 'none' }} />
            ...
            {availableModels.length > 0 ? (
              <select value={settings.model} onChange={(e) => setSettings({ ...settings, model: e.target.value })} style={{ background: '#000', color: '#ddd', border: `1px solid ${tc.surfaceLight}`, padding: '6px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.72rem', outline: 'none' }}>
                {availableModels.map(m => <option key={m} value={m}>{m}</option>)}
              </select>
            ) : (
              <input type="text" value={settings.model} onChange={(e) => setSettings({ ...settings, model: e.target.value })} placeholder="Model name" style={{ background: '#000', color: '#ddd', border: `1px solid ${tc.surfaceLight}`, padding: '6px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.72rem', outline: 'none' }} />
            )}
        ...
        <div>
          <div style={{ fontSize: '0.75rem', color: accent, fontWeight: 'bold', marginBottom: '8px', borderBottom: `1px solid ${tc.surfaceLight}`, paddingBottom: '4px' }}>SYSTEM PROMPT {customPrompt ? '(custom)' : '(default)'}</div>
          <textarea value={customPrompt || defaultPrompt} onChange={(e) => { setCustomPrompt(e.target.value); setPromptDirty(true) }} style={{ background: '#000', color: '#ddd', border: `1px solid ${tc.surfaceLight}`, padding: '6px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.68rem', outline: 'none', minHeight: '100px', resize: 'vertical', lineHeight: '1.4' }} />
          <div style={{ display: 'flex', gap: '4px', marginTop: '4px' }}>
            <button onClick={savePrompt} disabled={!promptDirty} style={{ flex: 1, background: promptDirty ? '#1a5a3a' : '#333', color: promptDirty ? '#5aff8a' : '#666', border: '1px solid #2a5a3a', padding: '5px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.7rem', cursor: promptDirty ? 'pointer' : 'not-allowed' }}>Save & Reset Chat</button>
            <button onClick={resetPrompt} style={{ background: '#3a1a1a', color: '#ff6666', border: '1px solid #5a2a2a', padding: '5px 10px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.7rem', cursor: 'pointer' }}>Reset</button>
          </div>
        </div>

        {/* UI Customization Section */}
        <div>
          <div style={{ fontSize: '0.75rem', color: accent, fontWeight: 'bold', marginBottom: '8px', borderBottom: `1px solid ${tc.surfaceLight}`, paddingBottom: '4px' }}>APPEARANCE</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
            <span style={{ color: '#888', fontSize: '0.7rem', width: '60px' }}>Theme:</span>
            {(['dark', 'light', 'matrix'] as const).map(t => (
              <button key={t} onClick={() => setUiSettings({ ...uiSettings, theme: t })} style={{ background: uiSettings.theme === t ? `${accent}30` : tc.surfaceLight, color: uiSettings.theme === t ? accent : '#666', border: `1px solid ${uiSettings.theme === t ? accent : tc.surfaceLight}`, padding: '3px 10px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.68rem', cursor: 'pointer' }}>{t}</button>
            ))}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
            ...
            <span style={{ color: '#aaa', fontSize: '0.7rem', width: '40px' }}>{uiSettings.fontSize.toFixed(2)}</span>
          </div>
        </div>

        {/* Command Timeout Section */}
        <div>
          <div style={{ fontSize: '0.75rem', color: accent, fontWeight: 'bold', marginBottom: '8px', borderBottom: `1px solid ${tc.surfaceLight}`, paddingBottom: '4px' }}>COMMAND TIMEOUT</div>
  ...
  const filesContent = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '6px 10px', background: tc.bg, borderBottom: `1px solid ${tc.surfaceLight}`, display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.72rem' }}>
        <button onClick={() => loadFiles('')} style={{ background: tc.surfaceLight, color: accent, border: `1px solid ${tc.surfaceLight}`, padding: '2px 8px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.7rem', cursor: 'pointer' }}>⟵ root</button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px' }}>
        {fileContent !== null ? (
          <div style={{ padding: '6px' }}>
            ...
            <button onClick={() => { setFileContent(null); setSelectedFile(null); setSelectedFilePath(''); loadFiles(currentDir) }} style={{ marginTop: '6px', background: tc.surfaceLight, color: accent, border: `1px solid ${tc.surfaceLight}`, padding: '3px 10px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.7rem', cursor: 'pointer' }}>⟵ back</button>
          </div>
        ) : files.map(f => (
    ...
    <div style={{ padding: '10px', fontSize: '0.74rem', fontFamily: 'monospace', color: '#aaa', height: '100%', overflowY: 'auto' }}>
      {state && (
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}><span>Alive:</span><span style={{ color: accent }}>{state.elapsed.toFixed(0)}s</span></div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}><span>Shell:</span><span style={{ color: state.shellAlive ? accent : '#ff4444' }}>{state.shellAlive ? 'ALIVE' : 'DEAD'}</span></div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}><span>BYOK:</span><span style={{ color: settings.apiKey ? accent : '#666' }}>{settings.apiKey ? settings.model || 'configured' : 'default z-ai'}</span></div>
  ...
  if (layout === 'mobile') {
    return (
      <div style={{ height: '100vh', background: tc.bg, color: accent, fontFamily: 'monospace', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontSize: `${uiSettings.fontSize}rem` }}>
        <div style={{ background: '#000', padding: '8px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${tc.surfaceLight}` }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: connected ? accent : '#ff4444', animation: 'pulse 1.5s infinite' }} />
          </div>
        </div>
        <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
          ...
          {activeMobileTab === 'settings' && settingsContent}
        </div>
        <div style={{ background: '#000', borderTop: `1px solid ${tc.surfaceLight}`, display: 'flex', padding: '6px 4px' }}>
          {[
            ...
            <button key={t.id} onClick={() => setActiveMobileTab(t.id)} style={{
              flex: 1, background: activeMobileTab === t.id ? `${accent}30` : 'transparent', color: activeMobileTab === t.id ? accent : '#666',
  ...
  return (
    <div style={{ minHeight: '100vh', background: tc.bgGradient, color: accent, fontFamily: 'monospace', overflow: 'hidden', display: 'flex', flexDirection: 'column', fontSize: `${uiSettings.fontSize}rem` }}>
      <div style={{ background: tc.bg, borderBottom: `1px solid ${tc.surfaceLight}`, padding: '6px 16px', display: 'flex', alignItems: 'center', gap: '1.5rem', fontSize: '0.78rem' }}>
        ...
        <button onClick={() => focusWindow('settings')} style={{ background: 'transparent', color: accent, border: `1px solid ${accent}55`, padding: '3px 12px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.7rem', cursor: 'pointer' }}>⚙ Settings</button>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', left: '8px', top: '8px', display: 'flex', flexDirection: 'column', gap: '10px', opacity: 0.4 }}>
          {[
            ...
            { label: 'Settings', win: 'settings', icon: '⚙' },
          ].map(icon => (
        ...
        {renderWindow(windows.find(w => w.id === 'settings')!, settingsContent)}
      </div>
      <div style={{ background: tc.bg, borderTop: `1px solid ${tc.surfaceLight}`, padding: '6px 16px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.72rem' }}>
        <span style={{ color: accent, fontWeight: 'bold' }}>◈</span>
        {windows.map(w => (
          <button key={w.id} onClick={() => focusWindow(w.id)} style={{ background: activeWin === w.id && !w.minimized ? `${accent}30` : tc.surfaceLight, color: activeWin === w.id && !w.minimized ? accent : '#888', border: `1px solid ${activeWin === w.id && !w.minimized ? accent : tc.surfaceLight}`, padding: '3px 10px', borderRadius: '3px', fontFamily: 'monospace', fontSize: '0.68rem', cursor: 'pointer' }}>{w.minimized ? `▸ ${w.title}` : w.title}</button>
        ))}
